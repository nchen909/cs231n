...
def affine_backward(dout, cache):
    ...
    dw = x.reshape(x.shape[0], np.prod(x.shape[1:])).T.dot(dout)#X^T*dout (D*N)*(N*M)
    #dL/dw=dY/dw*dL/dY= X^T*dout
    db = dout.sum(axis=0)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_mean = x.mean(axis=0)#attention! here is axis=0
        x_var = x.var(axis=0)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    #fulfilling batchnorm and layernorm is totally beyond my capacity..
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        out = x * mask #out= x/p if x[i,j]>=p else 0
...
def svm_loss(x, y):
    ...
    - y: Vector of labels, of shape (N,) where y[i] is the label for x[i] and
      0 <= y[i] < C
    for example,N=1000 C=7
    x[500][3]:500th sample label3 score
    y[500]=3 #truth lable
    Returns a tuple of:
    ...
    num_pos = np.sum(margins > 0, axis=1)#one label sum_loss
    dx = np.zeros_like(x)
    dx[margins > 0] = 1
    dx[np.arange(N), y] -= num_pos#correct class -sum loss
    dx /= N
    return loss, dx
